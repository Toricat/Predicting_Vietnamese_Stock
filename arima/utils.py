...
def acf_pacf_plot(data, name=None):
    ...
    plot_acf(data, ax=ax1, title=f'Autocorrelation(acf_plot): {name}')

    fig2, ax2 = plt.subplots()
    plot_pacf(data, ax=ax2, title=f'Partial Autocorrelation(pacf_plot): {name}')
    return fig1, fig2

def visualization_result(test_data,predictions,stock,title="Price",save="arima_model.pdf"):
    fig = plt.figure(figsize=(16,8))
    plt.plot(stock.index[-600:], stock.tail(600), color='green', label = 'Train Stock Price')
    plt.plot(test_data.index, test_data , color = 'red', label = 'Real Stock Price')
    plt.plot(test_data.index, predictions, color = 'blue', label = 'Predicted Stock Price')
    plt.title(f'Stock {title} Prediction')
    plt.xlabel('Time')
